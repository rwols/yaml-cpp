...
template <class Key, class T, class Compare = std::less<Key>>
using MockMultimap =
    std::multimap<Key, T, Compare, MockAllocator<std::pair<const Key, T>>>;

TEST_F(EmitParseDecodeTest, StdMultiMap) {
  MockMultimap<std::string, std::string> surnames2firstnames;
  ...
  std::multimap<std::string, std::string> samething(surnames2firstnames.begin(),
                                                    surnames2firstnames.end());
  check(surnames2firstnames);
  check(samething);
}

template <class Key, class T, class Hash = std::hash<Key>,
          class KeyEqual = std::equal_to<Key>>
using MockUnorderedMultimap =
    std::unordered_multimap<Key, T, Hash, KeyEqual,
                            MockAllocator<std::pair<const Key, T>>>;

TEST_F(EmitParseDecodeTest, StdUnorderedMultiMap) {
  MockUnorderedMultimap<std::string, std::string> surnames2firstnames;
  surnames2firstnames.insert(std::make_pair("Wilson", "Bilson"));
  surnames2firstnames.insert(std::make_pair("Wilson", "Brad"));
  surnames2firstnames.insert(std::make_pair("Wilson", "Amanda"));
  surnames2firstnames.insert(std::make_pair("Smith", "Agent"));
  ...
  surnames2firstnames.insert(std::make_pair("Doe", "John"));
  surnames2firstnames.insert(std::make_pair("Doe", "Kate"));
  std::unordered_multimap<std::string, std::string> samething(
      surnames2firstnames.begin(), surnames2firstnames.end());
  check(surnames2firstnames);
}
